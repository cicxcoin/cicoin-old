...
- The oldest supported Python version is specified in [doc/dependencies.md](/doc/dependencies.md).
  ...
  The Travis linter also checks this, but [possibly not in all cases](https://github.com/cicoin/cicoin/pull/14884#discussion_r239585126).
- See [the python lint script](/test/lint/lint-python.sh) that checks for violations that
  could lead to bugs and issues in the test code.
- Avoid wildcard imports
- Use a module-level docstring to describe what the test is testing, and how it
  is testing it.
- When subclassing the CicoinTestFramwork, place overrides for the
...
with the cicoind(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a cicoind.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the Cicoin Core node application logic. For custom behaviour, subclass the
...
Taken from the [python-cicoinrpc repository](https://github.com/jgarzik/python-cicoinrpc).
...
Basic code to support P2P connectivity to a cicoind.

#### [script.py](test_framework/script.py)
Utilities for manipulating transaction scripts (originally from python-cicoinlib)

# PSBT Howto for Cicoin Core

Since Cicoin Core 0.17, an RPC interface exists for Partially Signed Cicoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/cicoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Cicoin transactions that are not fully signed
...
#### Multisig with multiple Cicoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Cicoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `cicoin-cli` is used.
